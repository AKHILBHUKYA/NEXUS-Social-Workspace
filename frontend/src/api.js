...
export const getSocialProviders = () => request('/social/providers');
export const getSocialConnections = () => request('/social/connections');
export const connectSocial = (platform) => request(`/social/connect/${encodeURIComponent(platform)}`);
export const disconnectSocial = (platform) => request(`/social/connections/${encodeURIComponent(platform)}`, { method: 'DELETE' });
export const publishX = (text) => request('/social/x/post', { method: 'POST', body: JSON.stringify({ text }) });
export const publishMeta = (platform, text) => request(`/social/meta/${encodeURIComponent(platform)}/post`, { method: 'POST', body: JSON.stringify({ text }) });
export const createInstagramContainer = (imageUrl, caption) => request('/social/instagram/container', { method: 'POST', body: JSON.stringify({ imageUrl, caption }) });
export const publishInstagramContainer = (creationId) => request('/social/instagram/publish', { method: 'POST', body: JSON.stringify({ creationId }) });
export const sendWhatsAppCloud = (to, text) => request('/social/whatsapp/send', { method: 'POST', body: JSON.stringify({ to, text }) });

export const getSocialStatus = () => request('/social/status');
export const publishUnified = (platform, text, imageUrl) => request('/social/publish', { method:'POST', body: JSON.stringify({ platform, text, imageUrl }) });
